...
fn test_attack_invert_price_zero_result() {
    ...
    let vault = env.vault_balance();
    assert!(vault >= 55_000_000_000,
        "Vault must not decrease: vault={}", vault);
}
...
fn test_attack_invert_price_extreme_small_raw() {
    ...
    assert_eq!(engine_vault as u64, vault,
        "Conservation after extreme invert: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_multi_instruction_deposit_trade_atomic() {
    ...
    let result = env.svm.send_transaction(tx);
    assert!(result.is_ok(),
        "Multi-instruction deposit+trade should succeed: {:?}", result);

    // Verify both took effect
    let pos = env.read_account_position(user_idx);
    assert_eq!(pos, 1_000_000, "Position should be set from trade");
    let cap = env.read_account_capital(user_idx);
    assert!(cap > 1_000_000_000, "Capital should include additional deposit: cap={}", cap);
}
...
fn test_attack_withdraw_scale_minus_one_misaligned() {
    ...
    let cap = env.read_account_capital(user_idx);
    assert_eq!(cap, 5_000_000, "Capital unchanged after failed misaligned withdraw");
}
...
fn test_attack_liquidation_equity_exactly_zero() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after liquidation attempt: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_deposit_and_crank_same_slot_no_exploit() {
    ...
    let cap = env.read_account_capital(user_idx);
    assert_eq!(cap, 5_000_000_000,
        "Capital should equal total deposits after same-slot crank: cap={}", cap);

    // Conservation
    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot should equal sum of capitals: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_trade_crank_withdraw_rapid_sequence() {
    ...
    let withdraw_result = env.try_withdraw(&user, user_idx, cap as u64);
    assert!(withdraw_result.is_ok(),
        "Withdraw after close should succeed: {:?}", withdraw_result);

    // Conservation
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_price_whipsaw_between_cranks() {
    ...
    assert!(cap <= 10_000_000_000,
        "Capital should not increase: cap={}", cap);

    // Conservation
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after whipsaw: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_incremental_deposits_with_position() {
    ...
    let cap_after = env.read_account_capital(user_idx);
    assert!(cap_after >= 7_000_000_000,
        "Capital should be >= 8B after second deposit: cap={}", cap_after);
    ...
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot desync after incremental deposit: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_warmup_funding_interaction_no_double_count() {
    ...
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot desync after warmup+funding: c_tot={} sum={}", c_tot, sum);

    // Vault conservation
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Vault conservation: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_lp_position_aggregate_after_many_trades() {
    ...
    let lp_pos = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, -150_000,
        "LP position should be -150K: got {}", lp_pos);
    ...
    assert_eq!(lp_pos2, -50_000,
        "LP position after partial close: got {}", lp_pos2);
    ...
    assert_eq!(c_tot, sum,
        "c_tot desync after multi-user trades: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_circuit_breaker_exact_cap_boundary() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after circuit breaker clamping: engine={} vault={}", engine_vault, vault);

    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot conservation: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_trade_exact_margin_boundary_succeeds() {
    ...
    assert!(result.is_ok(),
        "Trade at exact margin boundary should succeed: {:?}", result);
...
fn test_attack_maintenance_fee_depletes_small_capital() {
    ...
    let user = Keypair::new();
    let user_idx = env.init_user(&user);
    env.deposit(&user, user_idx, 1_000_000); // Very small deposit (1M tokens)
    ...
    assert!(cap < 1_000_000,
        "Capital should be depleted by maintenance fees: cap={}", cap);
...
fn test_attack_mark_precision_small_increments() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after small increments: engine={} vault={}", engine_vault, vault);
...
fn test_attack_update_config_during_active_trades() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after config change: engine={} vault={}", engine_vault, vault);
...
fn test_attack_push_oracle_same_as_last_price() {
    ...
    let vault = env.vault_balance();
    assert_eq!(vault, 20_000_000_000,
        "Vault should not change from repeated same-price pushes: vault={}", vault);
}
...
fn test_attack_deposit_after_liquidation_same_slot() {
    ...
    // At least one of these operations should have happened (not both silently no-op)
    let something_happened = liq_result.is_ok() || deposit_result.is_ok() || deposit_result.is_err();
    assert!(something_happened, "At least one operation must have had an effect");

    // Conservation must hold either way
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after liquidation+deposit: engine={} vault={}", engine_vault, vault);

    // Verify vault changed if deposit succeeded (tokens transferred in)
    if deposit_result.is_ok() {
        assert!(vault > vault_before,
            "Deposit succeeded so vault must have increased: before={} after={}", vault_before, vault);
    }
}
...
fn test_attack_init_lp_matcher_is_self_program() {
    ...
    let tx = Transaction::new_signed_with_payer(
        &[ix], Some(&lp_owner.pubkey()), &[&lp_owner], env.svm.latest_blockhash(),
    );
    let result = env.svm.send_transaction(tx);
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after self-matcher init: engine={} vault={}", engine_vault, vault);
    // Verify the LP didn't extract any value
    assert_eq!(vault, 0, "No tokens should have been deposited: vault={}", vault);
}
...
fn test_attack_funding_rate_sign_flip_lp_position_cross() {
    ...
    let lp_pos = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, 5_000_000,
        "LP should have flipped to long: pos={}", lp_pos);
    ...
    let sum = env.read_account_capital(lp_idx) +
              ...
              env.read_account_capital(user2_idx);
    assert_eq!(c_tot, sum,
        "c_tot after LP position flip: c_tot={} sum={}", c_tot, sum);
}

// REMOVED: test_attack_gc_exact_threshold_account - duplicate of
// test_attack_user_gc_when_empty (round 5, line 9260)

// REMOVED: test_attack_close_account_fee_forgiveness_edge - duplicate of
// test_attack_close_account_fee_debt_forgiveness (round 9, line 14814)
...
fn test_attack_topup_insurance_unit_scale_dust() {
    ...
    // Insurance topup succeeds but 999 / 1000 = 0 units (dust lost to protocol)
    if result.is_ok() {
        let insurance = env.read_insurance_balance();
        // Insurance is 0 units because 999 < unit_scale=1000
        assert_eq!(insurance, 0,
            "Sub-unit-scale topup should round to 0: got {}", insurance);
    }

    // Now top up with aligned amount to verify it works
    env.try_top_up_insurance(&admin, 2000).unwrap(); // 2000 / 1000 = 2 units
    let insurance = env.read_insurance_balance();
    assert_eq!(insurance, 2,
        "Aligned topup should give 2 units: got {}", insurance);
...
fn test_attack_update_config_after_resolution() {
    ...
    // Verify market was actually resolved (precondition)
    // After resolution, attempt config update
    let result = env.try_update_config(&admin);
    // Record whether it succeeded
    let config_succeeded = result.is_ok();

    // Regardless of outcome, state must remain consistent
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after config update attempt: engine={} vault={}", engine_vault, vault);

    // If config update succeeded after resolution, verify no deposits allowed
    // (resolution blocks deposits even if config was modified)
    if config_succeeded {
        let user = Keypair::new();
        let user_idx_result = env.try_init_user(&user);
        assert!(user_idx_result.is_err(),
            "InitUser should fail after resolution, even with config update");
    }
}
...
fn test_attack_lp_position_oscillation() {
    ...
    assert_eq!(env.read_account_position(user_idx), expected_user_pos,
        "User position after oscillation");
    // LP pos = -200K
    assert_eq!(env.read_account_position(lp_idx), -expected_user_pos,
    ...
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot after oscillation: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_oracle_price_cap_u64_max() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation with disabled circuit breaker: engine={} vault={}", engine_vault, vault);
...
fn test_attack_set_fee_then_immediate_close() {
    ...
    // Set maintenance fee
    env.try_set_maintenance_fee(&admin, 100_000).unwrap();
    ...
    let close_result = env.try_close_account(&user, user_idx);
    assert!(close_result.is_ok(),
        "CloseAccount after fee set should succeed: {:?}", close_result);
...
fn test_attack_withdraw_between_two_cranks() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after withdraw between cranks: engine={} vault={}", engine_vault, vault);

    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot after withdraw between cranks: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_slot_reuse_multi_user_gc_reinit() {
    ...
    assert_eq!(c_tot, sum,
        "c_tot after slot reuse: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_risk_threshold_exact_insurance_boundary() {
    ...
    // Top up insurance to exact amount
    env.try_top_up_insurance(&admin, 5_000_000_000).unwrap();
    env.crank();
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation at threshold boundary: engine={} vault={}", engine_vault, vault);

    // If trade succeeded, verify position was opened
    if trade_result.is_ok() {
        let pos = env.read_account_position(user_idx);
        assert_eq!(pos, 1_000_000,
            "Trade succeeded so position must be 1M: got {}", pos);
    } else {
        // Trade rejected at boundary - verify no position opened
        let pos = env.read_account_position(user_idx);
        assert_eq!(pos, 0,
            "Trade rejected so position must be 0: got {}", pos);
    }
}
...
fn test_attack_lp_withdraw_during_haircut() {
    ...
    assert_eq!(engine_vault as u64, vault_after,
        "Conservation after LP withdrawal attempt: engine={} vault={}", engine_vault, vault_after);
    ...
    if withdraw_result.is_ok() {
        ...
        let lp_cap_after = env.read_account_capital(lp_idx);
        assert!(lp_cap_after < lp_cap_before,
...
fn test_attack_warmup_partial_close_vesting() {
    ...
    let user_pos = env.read_account_position(user_idx);
    assert_eq!(user_pos, 2_500_000,
        "User position after partial close: expected 2.5M, got {}", user_pos);
    ...
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot after warmup partial close: c_tot={} sum={}", c_tot, sum);
...
fn test_attack_mark_pnl_one_unit_position() {
    ...
    let pnl = env.read_account_pnl(user_idx);
    assert!(pnl >= 0, "Tiny PnL should not be negative for long: pnl={}", pnl);

    // Conservation
    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot with 1-unit position: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_haircut_zero_pnl_pos_tot() {
    ...
    let user_pnl = env.read_account_pnl(user_idx);
    assert!(user_pnl < 0,
        "Precondition: user should have negative PnL after price drop: {}", user_pnl);
    ...
    let user_cap = env.read_account_capital(user_idx);
    assert!(user_cap > 0,
        "Precondition: user should still have some capital: {}", user_cap);
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault_after,
        "Conservation with haircut: engine={} vault={}", engine_vault, vault_after);
}
...
fn test_attack_many_users_single_lp_conservation() {
    ...
    assert_eq!(c_tot, sum,
        "c_tot with 10 users: c_tot={} sum={}", c_tot, sum);

    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation with 10 users: engine={} vault={}", engine_vault, vault);
    ...
    assert_eq!(lp_pos, -user_sum,
        "LP position should mirror user sum: lp={} user_sum={}", lp_pos, user_sum);
}
...
fn test_attack_position_flip_margin_requirement_switch() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after flip attempt: engine={} vault={}", engine_vault, vault);

    // Verify position is consistent
    let pos = env.read_account_position(user_idx);
    if flip_result.is_ok() {
        assert_eq!(pos, -1_000_000,
            "Flip succeeded so position should be -1M: got {}", pos);
    } else {
        assert_eq!(pos, 1_000_000,
            "Flip failed so position should remain +1M: got {}", pos);
    }
}
...
fn test_attack_maintenance_fee_huge_dt_saturation() {
    ...
    env.try_set_maintenance_fee(&admin, u64::MAX as u128).unwrap();
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after huge dt fee: engine={} vault={}", engine_vault, vault);

    // User's capital should be depleted (or account GC'd)
    let user_cap = env.read_account_capital(user_idx);
    assert_eq!(user_cap, 0,
        "Huge fee should deplete user capital: cap={}", user_cap);
}
...
fn test_attack_rapid_successive_trades_accumulation() {
    ...
    let pos = env.read_account_position(user_idx);
    assert_eq!(pos, 1_750_000,
        "Rapid trades should accumulate: expected 1.75M, got {}", pos);
...
fn test_attack_three_lps_aggregate_tracking() {
    ...
    let user_pos = env.read_account_position(user_idx);
    assert_eq!(user_pos, 2_500_000,
        "User should have net 2.5M position: got {}", user_pos);
    ...
    let sum = env.read_account_capital(lp1_idx) +
              ...
              env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot across 3 LPs: c_tot={} sum={}", c_tot, sum);
...
fn test_attack_set_pnl_aggregate_rapid_flips() {
    ...
    assert!(pnl1 > 0, "User should have positive PnL after price rise: {}", pnl1);

    env.set_slot_and_price(400, 125_000_000); // Price crashes below entry
    env.crank();
    let pnl2 = env.read_account_pnl(user_idx);
    assert!(pnl2 < 0, "User should have negative PnL after price crash: {}", pnl2);

    env.set_slot_and_price(600, 150_000_000); // Recovery
    env.crank();
    let pnl3 = env.read_account_pnl(user_idx);
    assert!(pnl3 > 0, "User should have positive PnL after recovery: {}", pnl3);

    // pnl_pos_tot should reflect current positive PnL
    let ppt = env.read_pnl_pos_tot();
    assert!(ppt > 0,
        "pnl_pos_tot should be positive after recovery: {}", ppt);

    // Conservation
    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot after PnL flips: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_lp_partial_close_aggregate_update() {
    ...
    let lp_pos = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, -3_000_000,
        "LP position after partial close: expected -3M, got {}", lp_pos);
    ...
    let sum = env.read_account_capital(lp_idx) +
              ...
              env.read_account_capital(user2_idx);
    assert_eq!(c_tot, sum,
        "c_tot after LP partial close: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_settlement_ordering_mark_funding_fees() {
    ...
    // Set maintenance fee to create fee debt during settlement
    env.try_set_maintenance_fee(&admin, 10_000).unwrap();
    ...
    // Big price move + long time gap = mark settlement + funding + fees all at once
    env.set_slot_and_price(1000, 150_000_000);
    env.crank();

    // Verify settlement was consistent
    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot after combined settlement: c_tot={} sum={}", c_tot, sum);

    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after combined settlement: engine={} vault={}", engine_vault, vault);

    // User should have gained from mark but lost from fees
    let user_cap = env.read_account_capital(user_idx);
    // With position=5M, price move from 138 to 150 = +$12/unit
    // Mark PnL ~ 5M * 12 / 1 = +60 units (simplified)
    // Fees should have eaten some capital
    // Verify capital is reasonable (not negative/zero from fee-before-mark bug)
    assert!(user_cap > 0,
        "User should have positive capital after profitable mark settlement: cap={}", user_cap);
}
...
fn test_attack_inverted_market_large_swing_conservation() {
    ...
    assert!(pnl <= 0,
        "Long in inverted market should lose when raw price rises: pnl={}", pnl);

    // Conservation
    let c_tot = env.read_c_tot();
    let sum = env.read_account_capital(lp_idx) + env.read_account_capital(user_idx);
    assert_eq!(c_tot, sum,
        "c_tot in inverted market: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_opposing_positions_price_roundtrip() {
    ...
    let sum = env.read_account_capital(lp_idx) +
              ...
              env.read_account_capital(user2_idx);
    assert_eq!(c_tot, sum,
        "c_tot after price roundtrip: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_withdraw_all_with_open_position() {
    ...
    assert_eq!(pos, 5_000_000,
        "Position should remain after failed withdrawal: pos={}", pos);

    // Capital should be unchanged
    let cap = env.read_account_capital(user_idx);
    assert_eq!(cap, 10_000_000_000,
        "Capital should be unchanged after failed withdrawal: cap={}", cap);
}
...
fn test_attack_insurance_topup_from_non_admin() {
    ...
    if result.is_ok() {
        let insurance = env.read_insurance_balance();
        assert!(insurance > 0,
            "Insurance should have increased: {}", insurance);
    }
}
...
fn test_attack_large_price_drop_liquidation_conservation() {
    ...
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after large drop + liquidation: engine={} vault={}", engine_vault, vault);
    ...
    let lp_pnl = env.read_account_pnl(lp_idx);
    let lp_cap = env.read_account_capital(lp_idx);
    assert!(lp_pnl > 0 || lp_cap > 100_000_000_000,
        "LP should have gained from user's loss: pnl={} cap={}", lp_pnl, lp_cap);
}
...
fn test_attack_instruction_data_extra_trailing_bytes() {
    ...
    assert_eq!(cap, 0,
        "Capital should remain 0 after rejected trailing-bytes deposit: {}", cap);
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after extra-bytes instruction: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_trade_size_i128_min_boundary() {
    ...
    let pos = env.read_account_position(user_idx);
    assert_eq!(pos, 0,
        "Position should be 0 after rejected extreme trade: pos={}", pos);
}
...
fn test_attack_withdraw_all_redeposit_same_slot() {
    ...
    env.try_withdraw(&user, user_idx, 5_000_000_000).unwrap();
    let cap_mid = env.read_account_capital(user_idx);
    assert_eq!(cap_mid, 0, "Capital should be 0 after full withdraw: {}", cap_mid);

    env.deposit(&user, user_idx, 5_000_000_000);
    let cap_after = env.read_account_capital(user_idx);
    assert_eq!(cap_after, 5_000_000_000,
        "Capital should be restored after re-deposit: {}", cap_after);
...
fn test_attack_lp_close_with_matched_positions() {
    ...
    let lp_pos_after = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, lp_pos_after,
        "LP position should be unchanged after failed close: {} vs {}", lp_pos, lp_pos_after);
}
...
fn test_attack_trade_long_then_short_net_zero() {
    ...
    assert_eq!(pos_after_close, 0,
        "Position should be 0 after closing: {}", pos_after_close);

    // LP position should also be zero
    let lp_pos = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, 0, "LP position should be 0 after net-zero trades: {}", lp_pos);

    // Conservation
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after net-zero trades: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_lp_rapid_multi_user_matching() {
    ...
    let lp_pos = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, expected_lp_pos,
        "LP position after multi-user matching: expected={} got={}", expected_lp_pos, lp_pos);
    ...
    assert_eq!(c_tot, sum,
        "c_tot after multi-user LP matching: c_tot={} sum={}", c_tot, sum);
}
...
fn test_attack_deposit_withdraw_deposit_cycle_aggregates() {
    ...
    assert_eq!(c_tot_1, c_tot_3,
        "c_tot should be same after full cycle: before={} after={}", c_tot_1, c_tot_3);

    // Verify per-account capital
    let user_cap = env.read_account_capital(user_idx);
    assert_eq!(user_cap, 3_000_000_000,
        "User capital should be 3B after cycle: {}", user_cap);
}
...
fn test_attack_liquidation_boundary_precision() {
    ...
    assert!(vault as u128 >= c_tot + insurance,
        "Conservation at liquidation boundary: vault={} c_tot={} ins={}", vault, c_tot, insurance);

    // If user still has position, they must have margin
    if user_pos != 0 {
        assert!(user_cap > 0,
            "User with position should have positive capital: cap={}", user_cap);
    }
}
...
fn test_attack_oracle_timestamp_zero_then_crank() {
    ...
    assert!(result2.is_ok(),
        "Should be able to push valid price after ts=0: {:?}", result2);

    // State should be consistent
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after ts=0 oracle push: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_oracle_timestamp_i64_max_no_overflow() {
    ...
    assert!(result.is_ok(), "Oracle should accept ts=i64::MAX: {:?}", result);

    // Push another price - no backwards timestamp rejection means it works
    let result2 = env.try_push_oracle_price(&admin, 141_000_000, 1000);
    assert!(result2.is_ok(),
        "Should still push prices after ts=MAX: {:?}", result2);

    // No overflow, state consistent
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after ts=MAX oracle push: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_lp_deposit_settles_fee_debt_first() {
    ...
    // Open position so fees accrue
    env.trade(&user, &lp, lp_idx, user_idx, 3_000_000);

    // Advance time to accumulate fee debt
    env.set_slot(500);
    env.crank();

    let lp_cap_before = env.read_account_capital(lp_idx);

    // LP deposits more capital (should settle fees first)
    env.deposit(&lp, lp_idx, 5_000_000_000);

    let lp_cap_after = env.read_account_capital(lp_idx);

    // Capital increase should be <= deposit amount (fees eaten some)
    let increase = lp_cap_after - lp_cap_before;
    assert!(increase <= 5_000_000_000,
        "LP capital increase should be <= deposit: increase={}", increase);

    // Conservation
    let vault = env.vault_balance();
    let engine_vault = env.read_engine_vault();
    assert_eq!(engine_vault as u64, vault,
        "Conservation after LP deposit with fees: engine={} vault={}", engine_vault, vault);
}
...
fn test_attack_config_change_applied_to_next_instruction() {
    ...
    let user_cap = env.read_account_capital(user_idx);
    assert!(user_cap < 5_000_000_000,
        "High fee should have reduced user capital: cap={}", user_cap);
...
fn test_attack_rapid_admin_transfers() {
    ...
    let r1 = env.try_set_maintenance_fee(&admin1, 100);
    assert!(r1.is_err(), "Admin1 should be locked out");

    let r2 = env.try_set_maintenance_fee(&admin2, 100);
    assert!(r2.is_err(), "Admin2 should be locked out");

    let r3 = env.try_set_maintenance_fee(&admin3, 100);
...
fn test_attack_deposit_to_lp_wrong_owner() {
    ...
    let lp_cap = env.read_account_capital(lp_idx);
    assert_eq!(lp_cap, 0,
        "LP capital should be 0 after failed unauthorized deposit: cap={}", lp_cap);
}
...
fn test_attack_opposing_users_pnl_conservation() {
    ...
    assert_ne!(long_equity, short_equity,
    ...
    let lp_pos = env.read_account_position(lp_idx);
    assert_eq!(lp_pos, 0, "LP should still have zero net position: {}", lp_pos);
